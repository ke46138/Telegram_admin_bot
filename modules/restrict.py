...
from config import admins, allowed_chats, user_blacklist
import time
import traceback
...
def setup_handlers(bot):
    ...
    def only_admins(message):
        try:
            ...
            for i in range(len(result)):
                if result[i][0] not in admins and result[i][0] not in allowed_chats and result[i][0] not in muted_users and result[i][0] not in user_blacklist:
    ...
    def all_users(message):
        try:
            ...
            for i in range(len(result)):
                if result[i][0] not in admins and result[i][0] not in allowed_chats and result[i][0] not in muted_users:
                    bot.restrict_chat_member(
